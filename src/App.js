...
addTododays(tododay) {
	if(tododay){
		...
		obgs.active = false
			...
			let todooo = {tododays:tododays,tododaytask:[]};
					this.todoHi(todooo);
						}
...
todoHi(e){
			let todo = {tododays: e.tododays === undefined ? this.state.tododays : e.tododays,
...
toDoDaysActive(DayId){
	...
	const obg = asd.find(t=> t.id === DayId)
		...
		let todooo = {tododayActive:DayId,
									tododaytask: this.state.todos.filter((todo, index) => todo.activeDay === DayId)};
		this.todoHi(todooo);
}
...
addTodo(todoText) {
	if(this.state.tododayActive){
			...
			let tododaytask = this.state.tododaytask.slice();
					todos.push({	activeDay:this.state.tododayActive ,id: this.state.nextId, text: todoText});
					tododaytask.push({	activeDay:this.state.tododayActive ,id: this.state.nextId, text: todoText});
									this.setState({
										todos: todos,
										nextId: ++this.state.nextId,
										tododaytask: tododaytask
...
apdateTodo(todoText) {
	...
	// let todos = this.state.todos.slice();
	// let todosTrue = todos.map(function(item,i){
	// 								return item.id ===todoText.id && item.activeDay===todoText.activeDay;
	// 							});
	// let asd = todosTrue.map(function(item,i){
	// 					if(item ===true){	return i}
	// 				});
	// asd = asd.filter((todo, index) => todo !==undefined );
	// let	ann = asd[0];

	// todos[ann] = {activeDay:todoText.activeDay ,id: todoText.id , text: todoText.newText};
...
let todooo = {tododays: this.state.tododays.filter((todo, index) => todo.id !== id),
		todos: this.state.todos.filter((todo, index) => todo.activeDay !== id),
													...
													{this.state.tododays.map((tododays)=> {
																return(
																				...
																				<Tododays 	tododay={tododays} 
																					...
																					<li className = "new-to-do-days" onClick={() => this.addTododays(true)} ><span>+</span> </li>
							</ul>
						</div>

								<TodoInput todoText="" activeId={this.state.tododayActive} addTodo={this.addTodo} 
														toDoDaysActive={this.toDoDaysActive}/>
